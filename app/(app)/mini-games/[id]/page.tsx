        ...
        <Card className="space-y-6">
          ...
          {result ? (
            ...
            <section className="space-y-5">
              ...
              {spellingStage === 'write' && <><p className="text-sm font-bold text-muted">Type the words you remember in any order. The server will check your spelling.</p>{selectedSpellingWords.map((word, index) => <div key={word.id}><Input id={`spelling-${word.id}`} label={`Remembered word ${index + 1}`} value={wordResponses[word.id] ?? ''} onChange={(event) => setWordResponses((previous) => ({ ...previous, [word.id]: event.target.value }))} autoComplete="off" /></div>)}<Button type="button" className="w-full" onClick={() => void finishGame()} loading={submitting}><Trophy className="h-4 w-4" aria-hidden="true" />See my results</Button></>}
            </section>
          ) : (
