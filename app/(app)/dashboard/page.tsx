...
function AdminDashboard({ readerList, books, voiceProfiles, leaderboard }: { readerList: Child[] | null; books: Book[] | null; voiceProfiles: VoiceProfile[] | null; leaderboard: LeaderboardEntry[] | null }) {
  return (
    <>
      ...
      <Card><div className="mb-4 flex flex-wrap items-start justify-between gap-3"><div className="min-w-0"><p className="text-xs font-semibold uppercase tracking-[.16em] text-brand-500">Library pulse</p><h2 className="mt-1 text-xl font-bold text-brand-900">Recently available books</h2></div><Link href="/books" className="inline-flex min-h-11 shrink-0 items-center gap-1 text-sm font-semibold text-brand-600 hover:underline">Open library <ArrowRight className="h-4 w-4" aria-hidden="true" /></Link></div>{books?.length ? <div className="grid gap-3 sm:grid-cols-3">{books.slice(-3).reverse().map((book) => <Link key={book.id} href={`/books/${book.id}`} className="min-w-0 rounded-2xl border border-border bg-bg p-4 transition-all hover:-translate-y-1 hover:bg-white hover:shadow-md">{book.cover_image_url ? <ImageIcon className="h-6 w-6 text-brand-600" aria-hidden="true" /> : <BookOpen className="h-6 w-6 text-brand-600" aria-hidden="true" />}<h3 className="mt-3 break-words text-sm font-bold text-brand-900">{book.title}</h3><BookAttribution label={book.created_by_label} className="mt-1 text-xs" /><p className="mt-1 text-xs text-muted">{book.age_group} · {book.reading_level}</p></Link>)}</div> : <p className="text-sm text-muted">No books have been added yet.</p>}</Card>
